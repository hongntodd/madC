...
$xml = new SimpleXMLExtended('<?xml version="1.0" encoding="UTF-8"?><orders/>');

$xml->addAttribute('scheme','2.0');
$mydata = $xml->addChild('order');	
$orderID = $mydata->addAttribute('order_id', $order_id);

$task = $mydata->addChild('task');
$taskID = $task->addAttribute('id', $order_id);
$username = $task->username = NULL;
$username = $task->username->addCData('Hong Todd');

$bottlesize = $task->addChild('bottleSize', 10);
$printAmount = $task->addChild('printAmount',10);
...
$result .= "Use before 2016";

$ingredient = $task->ingredients = NULL;	
$ingredient = $task->ingredients ->addCData('Base: '.$result);
$liquids = $task->addChild('liquids');
