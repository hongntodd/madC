...
class WC_Cart {
	...
	/*-----------------------------------------------------------------------------------*/
		...
		/**count cart category if any product has term that belong to category **/
		public function count_cart_category($category_idd) {
			...
			foreach ( $woocommerce->cart->get_cart() as $cart_item_key => $values ) {
				...
				//if(in_array($category_idd, (array)$cat_ids) ) {//categories
				if( has_term( $category_idd, 'product_cat',$values['product_id']  ) ) {
				  //category is in cart!				 
	...
	/*-----------------------------------------------------------------------------------*/
		...
		public function add_to_cart( $product_id, $quantity = 1, $variation_id = '', $variation = array(), $cart_item_data = array() ) {
			...
			try {
				...
				//$product_price = $_product->get_price(); work but output as a string
				...
				if ( $product_data->is_sold_individually() ) {
					...
					if ( $in_cart_quantity > 0 ) {
						throw new Exception( sprintf( '<a href="%s" class="button wc-forward">%s</a> %s', $this->get_cart_url(), __( 'View Cart', 'woocommerce' ), sprintf( __( '&quot;%s&quot; is sold by individually', 'woocommerce' ), $product_data->get_title() ) ) );
					}
				}
				
				//get categories from $product_id
				global $post;
				$terms = wp_get_post_terms( $product_id, 'product_cat' );
				foreach ( $terms as $term ) $categories[] = $term->slug;
				
				
				//Count product is in strain category			
				$strain_qty = $this->count_cart_category(array( 'indica', 'sativa', 'hybrid'));				
				//Check product is in strain category	
				if ( in_array( 'sativa', $categories ) || in_array( 'indica', $categories )  || in_array( 'hybrid', $categories ) ) {
					$instrain = true;
				} else {$instrain = false;}
			
				if($strain_qty>=1 && $instrain === true) {
					throw new Exception( __( 'Only one Strain is allowed' ,'woocommerce') );		
				}
				
				
				//Count product is in flavors category			
				$flavors_qty = $this->count_cart_category(array(
					 'popular',
					 'smoke-shop',
					 'coffee-shop',
					 'open-bar',
					 ...
					 'fruits',
					 'floral-shop'));
				//Check product is in flavors category		 
				if ( in_array( 'popular', $categories ) 
					|| in_array( 'smoke-shop', $categories )  
					|| in_array( 'coffee-shop', $categories )  
					|| in_array( 'open-bar', $categories )  
					|| in_array( 'soda-fountain', $categories )  
					|| in_array( 'sweets-treats', $categories )  
					|| in_array( 'fruits', $categories )  
					|| in_array( 'floral-shop', $categories ) ) {
						$inflavors = true;
				} else {$inflavors = false;}
				
				if($flavors_qty >= 5 && $inflavors === true) {
					throw new Exception( __( 'No more than five Flavors are allowed' ,'woocommerce') );		
				}
				
				
				//Count product is in THC	
				$thc_qty = $this->count_cart_category(array( 'thc'));				
				//Check product is in thc
				if ( in_array( 'thc', $categories ) ) {	$inthc = true;} else {$inthc = false;}
			
				if($thc_qty>=1 && $inthc === true) {
					throw new Exception( __( 'Only one THC Level is allowed' ,'woocommerce') );		
				}
				
				//Count product is in CBD
				$cbd_qty = $this->count_cart_category(array( 'cbd'));				
				//Check product is in CBD
				if ( in_array( 'cbd', $categories ) ) {	$incbd = true;} else {$incbd = false;}
			
				if($cbd_qty>=1 && $incbd === true) {
					throw new Exception( __( 'Only one CBD Level is allowed' ,'woocommerce') );		
				}
				
				//Count product is in Nicotine
				$nicotine_qty = $this->count_cart_category(array( 'nicotine'));				
				//Check product is in Nicotine
				if ( in_array( 'nicotine', $categories ) ) {	$innicotine = true;} else {$innicotine = false;}
			
				if($nicotine_qty>=1 && $innicotine === true) {
					throw new Exception( __( 'No more than '.$nicotine_qty. ' from Nicotine is allowed' ,'woocommerce') );		
				}
